from collections.abc import Mapping
from typing import Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field

T = TypeVar("T", bound="CocktailRecommendationModel")


@_attrs_define
class CocktailRecommendationModel:
    """The cocktail recommendation model"""

    name: str
    """ The name of the cocktail or variation """
    ingredients: str
    """ The ingredients description of the cocktail or variation """
    directions: str
    """ The directions description of the cocktail or variation """
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        name = self.name

        ingredients = self.ingredients

        directions = self.directions

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "name": name,
                "ingredients": ingredients,
                "directions": directions,
            }
        )

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        name = d.pop("name")

        ingredients = d.pop("ingredients")

        directions = d.pop("directions")

        cocktail_recommendation_model = cls(
            name=name,
            ingredients=ingredients,
            directions=directions,
        )

        cocktail_recommendation_model.additional_properties = d
        return cocktail_recommendation_model

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
